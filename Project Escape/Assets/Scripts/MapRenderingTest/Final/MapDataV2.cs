...
public class MapDataV2
{
    ...
    public MapDataV2(int[,] mapCells, Vector2Int start_coordinates, Vector2Int end_coordinates, List<Actor> enemies)
    {
    ...
    private bool IsPositionOcupiedByAnEnemy(Vector2Int pos)
    {
        ...
        {
            if (pos.x == a.GetActor().GetComponent<EnemyControllerV2>().GetCellPosition().x &&
                pos.y == a.GetActor().GetComponent<EnemyControllerV2>().GetCellPosition().y)
            {
                return true;
            }
        }
    ...
    {
        return (pos.x == GameManager.manager.player.GetActor().GetComponent<PlayerControllerV2>().GetCellPosition().x &&
           pos.y == GameManager.manager.player.GetActor().GetComponent<PlayerControllerV2>().GetCellPosition().y);
    }
    ...
    public Actor GetEnemyInCellIfThereIs(Vector2Int cellPosition)
    {
        foreach (Actor a in laEnemies)
        {
            if (cellPosition.x == a.GetActor().GetComponent<EnemyControllerV2>().GetCellPosition().x &&
                cellPosition.y == a.GetActor().GetComponent<EnemyControllerV2>().GetCellPosition().y)
            {
                return a;
            }
        }
